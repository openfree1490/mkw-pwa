...
**Project:** MKW Command Center (mkw-pwa)
**Platform:** FastAPI (Python) backend + React (Vite) frontend PWA
...
No .env files found in the repository. Environment variables are set via Railway deployment configuration.

Expected environment variables (from code analysis):
```
...
## 1d. Main App Entry Point, Router Config, Layout/Navigation
...
<html lang="en">
  <head>
    ...
    <meta name="apple-mobile-web-app-title" content="MKW" />
    ...
    <title>MKW Command Center</title>

    <style>
      * { box-sizing: border-box; margin: 0; padding: 0; }
      html, body, #root {
        height: 100%; width: 100%;
        background: #020408; color: #b8c4d8;
        overflow: hidden;
      ...
      ::-webkit-scrollbar-thumb { background: #1b2a40; border-radius: 2px; }
      ::-webkit-scrollbar-thumb:hover { background: #243856; }
      /* Scanline overlay */
      #root::after {
        content: '';
        position: fixed;
...
### src/App.jsx (Main Application — 2616 lines)
...
No database schema (Prisma, SQL, etc.) exists. All data is:
- Fetched live from APIs (Polygon, Finnhub, yfinance, FRED, FINRA)
- Cached in-memory (Python dicts with TTL)
- Persisted to /tmp JSON files (positions, journal, archive)
...
## 2a. Polygon Integration — backend/polygon_client.py (672 lines)
...
## 2b. Finnhub Integration

Finnhub is used directly in `backend/main.py` via the `finnhub_get()` helper function.
Used for: general market news, company news, and earnings calendar.
Relevant code (from main.py lines 162-171, 926-979):
...
## 2c. FRED / St. Louis Fed — backend/macro_engine.py (477 lines)
...
## 2d. FINRA Short Volume — backend/finra_short_volume.py (338 lines)
...
No dedicated UnusualWhales or UW_API integration. Unusual options activity is detected
locally via volume/OI ratio analysis in `backend/options_engine.py` (lines 811-829).

## 2f. Data Router / Cache Layer — backend/data_router.py (353 lines)
...
## 2g. Server-Side Routes (FastAPI) — backend/main.py (2631 lines)

The full FastAPI backend with all API endpoints. Full file contents:
...
## 3a. Scoring / Convergence / Grading — backend/grading.py (568 lines)
...
### Convergence scoring (in main.py, function convergence_score, lines 390-420)

Included in full main.py above in Section 2g.

### Challenge Engine Scoring — src/challenge/engine/scoring.js (92 lines)
...
## 3b. Weinstein Stage Detection

Defined in `backend/main.py` function `weinstein_stage()` (lines 194-231).
Included in full main.py above in Section 2g.

## 3c. Minervini Trend Template

Defined in `backend/main.py` function `minervini_template()` (lines 233-250).
Included in full main.py above in Section 2g.

## 3d. Kell EMA Phase

Defined in `backend/main.py` function `kell_phase()` (lines 252-316).
Included in full main.py above in Section 2g.

## 3e. Qullamaggie Setup Detection — backend/qullamaggie.py (711 lines)
...
### Challenge Detection Engine — src/challenge/engine/detection.js (218 lines)
...
## 3f. Technical Indicator Calculations

### backend/indicators.py (149 lines)
...
### Challenge Analysis Engine — src/challenge/engine/analysis.js (256 lines)
...
## 3g. Relative Strength Calculations

Defined in `backend/main.py` function `calc_rs_rating()` (lines 184-192).
Included in full main.py above.

## 3h. Volume Analysis

Volume analysis is embedded in:
- `backend/main.py` `calc_technicals()` (OBV, volume profile)
- `backend/main.py` `detect_vcp()` (volume dry-up detection)
- `backend/qullamaggie.py` (volume surge/climax detection)
All included in full file contents above.

---

# SECTION 4: OPTIONS ENGINE

## 4a. Options Pricing, IV, Greeks — backend/options_engine.py (912 lines)
...
## 4b. Strategy Selection & Trade Ideas — backend/trade_ideas.py (770 lines)
...
## 4c. IV Rank / IV Percentile

Implemented in:
- `backend/options_engine.py` `calc_iv_from_options_chain()` (lines 185-389)
- `backend/polygon_client.py` `calc_iv_rank_from_history()` and `extract_iv_analysis_from_snapshot()`
Both included in full file contents above.

## 4d. Options Flow / Unusual Activity

Detected in `backend/options_engine.py` `full_options_analysis()` lines 811-829.
Volume/OI ratio > 2 flagged as unusual. Included in full file above.

---

# SECTION 5: WATCHLIST, JOURNAL, RISK & TOOLS

## 5a. Watchlist — src/challenge/tabs/WatchlistTab.jsx (311 lines)
...
## 5b. Trade Journal — backend/journal.py (528 lines)
...
### Trade Journal UI — src/challenge/tabs/TradesTab.jsx (323 lines)
...
## 5c. Risk Module — src/challenge/tabs/RiskTab.jsx (212 lines)
...
### Position Sizer — src/challenge/tabs/SizerTab.jsx (186 lines)
...
### Trade Rules — backend/trade_rules.py (215 lines)
...
## 5d. Daily Brief / Market Summary

Implemented in `backend/main.py` functions:
- `generate_programmatic_brief()` (lines 984-1118)
- `generate_daily_brief()` (lines 1120-1184)
Both included in full main.py above.

## 5e. Pre-Trade Checklist

The convergence checklist serves as the pre-trade checklist.
See `convergence_score()` with `detailed=True` in main.py (lines 390-420).
Also see frontend convergence checklist rendering in App.jsx.

## 5f. Scanner / Screener

Screener presets defined in `backend/main.py` `SCREENER_PRESETS` dict (lines 1189-1266).
Scanner modules defined in `backend/main.py` `SCANNER_MODULES` dict (lines 1271-1302).
Scanner execution in `_run_scanner()` function (lines 1305-1391).
All included in full main.py above.
...
```
 15185 total
  ...
  2616 ./src/App.jsx
   ...
   339 ./backend/wizard.py
   338 ./backend/finra_short_volume.py
   323 ./src/challenge/tabs/TradesTab.jsx
   311 ./src/challenge/tabs/WatchlistTab.jsx
   310 ./backend/llm_provider.py
   ...
   149 ./backend/indicators.py
   114 ./src/challenge/components/shared.jsx
```

## 6d. Remaining Large Files (>200 lines, not yet included)

### LLM Provider — backend/llm_provider.py (310 lines)
...
### Market Wizard Chat — backend/wizard.py (339 lines)
...
### Challenge App Container — src/challenge/ChallengeApp.jsx (281 lines)
...
### Debrief Tab — src/challenge/tabs/DebriefTab.jsx (286 lines)
...
### Ideas Tab — src/challenge/tabs/IdeasTab.jsx (242 lines)
...
### Command Tab — src/challenge/tabs/CommandTab.jsx (234 lines)
...
### Engine Constants — src/challenge/engine/constants.js (273 lines)
...
### Trade Builder — src/challenge/engine/tradeBuilder.js (53 lines)
...
### Playbook Tab — src/challenge/tabs/PlaybookTab.jsx (150 lines)
...
### CandleChart Component — src/challenge/components/CandleChart.jsx (156 lines)
...
### Shared Components — src/challenge/components/shared.jsx (114 lines)
...
### MetricCard — src/challenge/components/MetricCard.jsx (36 lines)
...
### ProgressBar — src/challenge/components/ProgressBar.jsx (36 lines)
...
### useStorage Hook — src/challenge/hooks/useStorage.js (40 lines)
...
### src/challenge/engine/constants.js (273 lines) — Challenge constants, tiers, scoring thresholds

Already included in Section 6d above.

## 7b. Scoring Thresholds & Grading Criteria

Defined in:
- `backend/grading.py` `GRADE_SCALE` (lines 16-23) — AAA to F grade scale
- `backend/main.py` `convergence_score()` — Convergence zone thresholds (17/12/8)
- `backend/main.py` `SCREENER_PRESETS` — Screener filter criteria
- `src/challenge/engine/constants.js` `TIERS` — $5K challenge tier definitions
All included in full file contents above.

## 7c. Color System / Theme

### Main App Theme (from src/App.jsx, lines 7-17):
...
### Challenge Theme (from src/challenge/engine/constants.js):

```js
// CC = Challenge Colors — defined in constants.js
export const CC = {
  bg: '#0a0e14', surface: '#111822', raised: '#1a2332',
  border: '#1e2a3a', borderHi: '#2a3a4a',
  accent: '#00b4d8', profit: '#00e676', loss: '#ff3d71', warning: '#ffaa00',
  textBright: '#e2e8f0', textMuted: 'rgba(180,200,220,0.5)',
  gold: '#ffd700', tier1: '#00e676', tier2: '#00b4d8', tier3: '#a855f7', tier4: '#ffd700',
}
```
...
### Main App Fonts:
```
Orbitron (headings/numbers) — futuristic display font
Rajdhani (body text) — clean technical font
Share Tech Mono (monospace/data) — monospace for values
```

### Challenge Module Fonts:
```
...
## Largest JSX file: src/App.jsx (2616 lines)

Already included in Section 1d above.

## Files with "engine" or "analysis" or "compute" in name/path

All included above:
- `src/challenge/engine/analysis.js` (256 lines)
- `src/challenge/engine/constants.js` (273 lines)
- `src/challenge/engine/detection.js` (218 lines)
- `src/challenge/engine/scoring.js` (92 lines)
- `src/challenge/engine/tradeBuilder.js` (53 lines)
- `backend/macro_engine.py` (477 lines)
- `backend/options_engine.py` (912 lines)

## Server entry point: backend/main.py (2631 lines)

Already included in Section 2g above.

## Middleware / Auth / Proxy

CORS middleware configured in main.py line 1440:
```python
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_methods=["*"], allow_headers=["*"])
```
No authentication layer. No proxy middleware beyond Vite dev proxy.

## Python Scripts

All Python files are in `backend/` directory. All 14 files have been fully included above.
